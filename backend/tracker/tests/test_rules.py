...
class VisitPhotoTest(TestCase):
    ...
    def test_contract_is_net_and_vat_is_derived(self):
        ...
        self.assertEqual(np_.contract_status, "draft")
        self.err("forbidden", billing.update_commercials, u["u_ft1"], np_.id, {"contractValueNet": 2_000_000})
        self.err("invalid", billing.update_commercials, u["u_fin"], np_.id, {"approvedBudget": 5_000_000})
        self.err("invalid", billing.update_commercials, u["u_fin"], np_.id, {"retentionPercent": 25})
        np_ = billing.update_commercials(u["u_fin"], np_.id, {"contractValueNet": 2_000_000, "vatTreatment": "withheld_by_client", "approvedBudget": 1_500_000, "retentionPercent": 10})
        self.assertEqual(np_.vat_amount, Decimal("150000.00")); self.assertEqual(dec(np_.contract_value), Decimal("2150000.00"))
        self.assertEqual(dec(np_.approved_budget), 1_500_000); self.assertEqual(dec(np_.retention_percent), 10)
        self.assertTrue(np_.events.filter(event_type="contract_updated").exists())
        serializers.project(np_)
        self.err("forbidden", billing.set_contract_status, u["u_ft1"], np_.id, {"status": "received"})
        doc = documents.add_document(u["u_fin"], np_.id, {"docType": "contract", "title": "Signed contract", "fileName": "contract.pdf", "sizeBytes": 1})
        np_ = billing.set_contract_status(u["u_fin"], np_.id, {"status": "received", "receivedOn": "2026-09-22", "documentId": doc.id})
        self.assertEqual(np_.contract_status, "received"); self.assertEqual(np_.contract_received_on.isoformat(), "2026-09-22"); self.assertEqual(np_.contract_received_by_id, "u_fin"); self.assertEqual(np_.contract_document_id, doc.id)
        self.assertEqual(serializers.project(np_)["contractReceivedOn"], "2026-09-22")
        self.err("conflict", billing.set_contract_status, u["u_fin"], np_.id, {"status": "received"})
        self.assertTrue(np_.events.filter(event_type="contract_received").exists())
        np_ = billing.set_contract_status(u["u_fin"], np_.id, {"status": "draft"})
        self.assertEqual(np_.contract_status, "draft"); self.assertIsNone(np_.contract_received_by_id)
        signed = projects.create_project(u["u_pm1"], {"name": "Signed job", "clientName": "Acme", "branchName": "HQ", "location": "Lagos", "projectType": "solar_battery",
                                                     "contractValueNet": 100, "contractReceived": True, "pmId": "u_pm1", "leadEngineerId": "u_pm2"})
        self.assertEqual(signed.contract_status, "received"); self.assertEqual(signed.contract_received_by_id, "u_pm1")
        # retention on net
