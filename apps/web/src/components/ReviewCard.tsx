    ...
    <article className="ns-approval ns-approval--pending">
      ...
      <div className="review__body">
        ...
        <div className="review__kv">
          ...
          {iss && <><span>Severity</span><b>{iss.severity} · SLA {api.issueSla(iss).breached ? "BREACHED" : `${api.issueSla(iss).hoursLeft} h`}</b><span>Description</span><b>{iss.description}</b><span>Photos</span><b>{iss.beforeAttachmentIds.length} before · {iss.afterAttachmentIds.length} after</b>
          ...
          {com && <><span>Result</span><b style={{ textTransform: "uppercase" }}>{com.result}</b><span>Checklist</span><b>{com.items.filter((i) => i.pass).length}/{com.items.length} pass{com.items.some((i) => !i.pass) ? ` — fails: ${com.items.filter((i) => !i.pass).map((i) => i.label).join(", ")}` : ""}</b>
            <span>Meter integrity</span><b>{(Object.keys(com.meter) as (keyof typeof com.meter)[]).map((k) => `${k} ${com.meter[k] ? "✓" : "✗"}`).join(" · ")}</b>{com.clientWitness && <><span>Witness</span><b>{com.clientWitness.name}</b></>}<span>On check</span><b>generates gate-5 evidence documents</b></>}
          {hse && <><span>Type</span><b>{HSE_TYPE_LABEL[hse.type]} · {hse.severity}</b><span>What happened</span><b>{hse.description}</b><span>Actions</span><b>{hse.actions}</b></>}
