 ...
 */
export function VoidControl({ kind, id, projectId, size = "sm", onDone }: { kind: VoidKind; id: string; projectId?: string; size?: "sm" | "xs"; onDone?: () => void }) {
  ...
  return <span className="void__form" onClick={(e) => e.stopPropagation()}>
    <input className="ns-input" style={{ minHeight: 30, width: 260 }} placeholder="Why is this wrong? (required)" value={why} autoFocus onChange={(e) => setWhy(e.target.value)} onKeyDown={(e) => { if (e.key === "Escape") setOpen(false); }} />
    <button type="button" className="ns-btn ns-btn--danger ns-btn--sm" disabled={!why.trim()} onClick={() => { if (safe(() => api.voidRecord(user.id, kind, id, why), `Voided — ${VOID_KIND_LABEL[kind]} no longer counts`)) { setOpen(false); setWhy(""); onDone?.(); } }}>Void</button>
